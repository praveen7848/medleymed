...
export default class SetAssessmentResponse extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	getAssessmentInfo(handle) {
		...
		.then((response) => {
			this.setState({
				assessmentScore: response.data.assessment_response
			});
		 })
		 .catch((error) => {
			 console.log(error);
									...
									<div className="container">
										<div className="row">
											<div className="tab-header">
												<h3>Set Assessment Response</h3>
											</div>
											<div id="reg_form">
												<form onSubmit={this.checkSubmit.bind(this)}>
													<div className="row">
														<div className="form-group col-md-4">
															<input type='button' className="margin15 txtBold" value='Add Response' onClick={this.addClick.bind(this)}/>
														</div>
													</div>
													{this.createUI()}
													<div className="row">
														<div className="form-group col-md-5">
															<button type="submit" className="btn  btn-primary padTopCategorySave fright" >Save Response</button> &nbsp; 
