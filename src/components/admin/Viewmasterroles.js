...
export default class ViewMasterRoles extends Component {
  constructor(props) {
    super(props);
    this.state = {
      page_data: "",
    ...
    this.deleteMasterRole = this.deleteMasterRole.bind(this);
  }
  ...
  fetchpagesdata() {
    Httpconfig.httptokenget(Constant.siteurl + "api/masterRoles")
      .then((response) => {
        this.setState({
          ...
          assignvalues.push({
            ...
            actions: (
              <div>
                <span>
                  <Link
                    to={"/admin/Createmasterroles/" + item.id}
                    className="fa fa-edit point-cursor"
                    title={"Update " + item.role_name}
                  ></Link>
                </span>{" "}
                &nbsp;
                <span>
                  <i
                    onClick={() => this.deleteMasterRole(item.id, item.role_name)}
                    className="fa fa-trash point-cursor"
                    title={"Delete " + item.role_name}
                  ></i>
                </span>{" "}
                &nbsp;
              </div>
  ...
  //delete controller
  deleteMasterRole(masterRoleId, name) {
    var isConfirm = window.confirm("Are you sure to delete " + name + "?");
    if (isConfirm) {
      Httpconfig.httptokendelete(Constant.siteurl + "api/masterRoles/" + masterRoleId)
        //axios.delete(Constant.siteurl+'api/Users/'+masterRoleId)
        .then((response) => {
          toast.success("Successfully Deleted Master role");
